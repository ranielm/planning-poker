  ...
  return (
    <div className="bg-slate-800/80 backdrop-blur rounded-xl p-6 border border-slate-700">
      ...
      <div className="text-center mb-6">
        {results.isConsensus ? (
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-white">{results.average}</p>
            <p className="text-slate-400 text-sm mt-1">
              Suggested: <span className="text-primary-400 font-medium">{results.roundedAverage}</span>
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <p className="text-sm text-slate-400 mb-3">Vote Distribution</p>
        {Object.entries(results.distribution)
          .sort(([a], [b]) => Number(a) - Number(b))
          .map(([value, count]) => (
            <div key={value} className="flex items-center gap-3">
              <span className="w-8 text-right font-medium text-slate-300">{value}</span>
      ...
      <div className="text-center mb-6">
        {results.isConsensus ? (
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-white">{results.mode}</p>
            <p className="text-slate-400 text-sm mt-1">
        ...
        {sizeOrder
          ...
          .map((size) => (
            <div key={size} className="flex items-center gap-3">
              <span className="w-10 text-right font-medium text-slate-300">{size}</span>
              <div className="flex-1 h-6 bg-slate-700 rounded overflow-hidden">
                <div
                  className={clsx(
                    'h-full transition-all duration-500',
              ...
              <span className="w-8 text-slate-400">{results.distribution[size]}</span>
            </div>
          ))}
      </div>
